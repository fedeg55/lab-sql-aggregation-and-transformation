...
SELECT *, date_format(CONVERT(rental_date,date), '%M') AS 'Month', date_format(CONVERT(rental_date,date), "%W") AS "Day" FROM rental LIMIT 20;
...
SELECT COUNT(*) AS films_released_by_2023 FROM film WHERE release_year <= 2023;

-- 1.2 The number of films for each rating.
SELECT rating, COUNT(*) AS number_of_films_by_rating FROM film GROUP BY rating;
...
SELECT rating, COUNT(*) AS number_of_films_by_rating FROM film GROUP BY rating ORDER BY number_of_films_by_rating DESC;
...
SELECT staff_id, COUNT(*) AS number_of_films_rented_by_employee FROM rental GROUP BY staff_id;
...
SELECT rating, ROUND(AVG(length),2) as average FROM film GROUP BY rating;
...
SELECT last_name, COUNT(*) AS frequency FROM actor GROUP BY last_name HAVING frequency = 1;